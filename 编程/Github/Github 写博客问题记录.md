...
如果你在 markdown 文件中插入图片：`![image](relative path/image.png)`，这在 Typora 和 VS Code 中或许能正常显示，但是在 Github 上只会显示原 markdown 代码。

因为 Github 路径是不允许空格的，必须使用百分比编码转义：`![image](relative%20path/image.png)`。
